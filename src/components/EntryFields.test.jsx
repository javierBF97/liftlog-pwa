...
describe('EntryFields', () => {
  ...
  it('seeds fields from `initial`', async () => {
    ...
    expect(screen.getByLabelText(/peso/i)).toHaveValue(100);
  ...
  it('gymnastics shows the time field only for accumulated', async () => {
    ...
    expect(screen.queryByLabelText(/minutos/i)).not.toBeInTheDocument();
    await userEvent.selectOptions(screen.getByLabelText(/modalidad/i), 'accumulated');
