...
export default function TimeSelect({ value, onChange }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
      <select aria-label="Minutos" value={m} onChange={(e) => emit(Number(e.target.value), s)} style={sel}>
        {MINS.map((n) => <option key={n} value={n}>{n}</option>)}
      </select>
      <span style={{ color: 'var(--muted)', fontSize: 13 }}>min</span>
      <select aria-label="Segundos" value={s} onChange={(e) => emit(m, Number(e.target.value))} style={sel}>
        {SECS.map((n) => <option key={n} value={n}>{String(n).padStart(2, '0')}</option>)}
      </select>
      <span style={{ color: 'var(--muted)', fontSize: 13 }}>seg</span>
    </div>
  );
}
