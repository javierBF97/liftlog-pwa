...
describe('PlatesManager', () => {
  it('lists current plates and adds a new one', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/plate weight/i), '12');
  ...
  it('removes a plate', async () => {
    ...
    await userEvent.click(screen.getAllByRole('button', { name: /remove plate/i })[0]);
  ...
  it('picks a preset color from the popover (no full palette shown by default)', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/plate weight/i), '8');
    await userEvent.click(screen.getByRole('button', { name: /plate color/i }));
  ...
  it('creates a custom color from the shade grid', async () => {
    const onAdd = vi.fn();
    render(<PlatesManager plates={DEFAULT_PLATES} onAdd={onAdd} onRemove={() => {}} />);
    await userEvent.type(screen.getByLabelText(/plate weight/i), '7');
    await userEvent.click(screen.getByRole('button', { name: /plate color/i }));
    const options = screen.getAllByRole('option');
  ...
  it('edits an existing plate and saves via onUpdate', async () => {
    ...
    await userEvent.click(screen.getAllByRole('button', { name: /edit plate/i })[0]);
